...
export class Facet<Input, Output = readonly Input[]> {
    ...
    /**
     * Shorthand method for registering a facet source with a state field as input. If the
     * field's type corresponds to this facet's input type, the getter function can be omitted.
     * If given, it will be used to retrieve the input from the field value.
     */
...
type StateFieldSpec<Value> = {
    ...
    /**
     * Compare two values of the field, returning `true` when they are the same. This is
     * used to avoid recomputing facets that depend on the field when its value did not
     * change. Defaults to using `===`.
     */
    compare?: (a: Value, b: Value) => boolean,

    /**
     * Provide extensions based on this field. The given function will be called once with
     * the initialized field. It will usually want to call some facet's {@link Facet.from} method
...
function flatten(extension: Extension, compartments: Map<Compartment, Extension>, newCompartments: Map<Compartment, Extension>) {
    ...
    function inner(ext: Extension, prec: number) {
        ...
        if (Array.isArray(ext)) {
            ...
            if (!content) throw new Error(`Unrecognized extension value in extension set (${ext}). This sometimes happens because multiple instances of state module are loaded, breaking instanceof checks.`)
            inner(content, prec)
        }
    }
