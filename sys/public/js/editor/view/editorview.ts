import {EditorState, Transaction, TransactionSpec, Extension, Prec, ChangeDesc, EditorSelection,
        SelectionRange, StateEffect, Facet, Line} from "../state/index"
import {StyleModule, StyleSpec} from "../utils/style-mod"
...
import {ViewUpdate, styleModule, contentAttributes, editorAttributes, AttrSource, clickAddsSelectionRange,
        dragMovesSelection, mouseSelectionStyle, exceptionSink, updateListener, logException, viewPlugin,
        ViewPlugin, PluginInstance, decorations, atomicRanges, scrollMargins, MeasureRequest, editable,
        inputHandler, perLineTextDirection, scrollIntoView, UpdateFlag, ScrollTarget} from "./extension"
...
interface EditorConfig {
    ...
    /**
     * Override the transaction [dispatch function]{@link EditorView.dispatch} for this editor view, which
     * is the way updates get routed to the view. Your implementation, if provided, should probably call the
     * view's [`update` method]{@link EditorView.update}.
     */
...
export class EditorView {
    ...
    /**
     * The editable DOM element holding the editor content. You should not, usually, interact
     * with this content directly though the DOM, since the editor will immediately undo most
     * of the changes you make. Instead, [dispatch]{@link EditorView.dispatch}
     * [transactions]{@link Transaction} to modify content, and [decorations]{@link Decoration}
     * to style it.
     */
 ...
 * this will infer the event argument type to hold the appropriate event object type. For unknown
 * events, it is inferred to `any`, and should be explicitly set if you want type checking.
 */
