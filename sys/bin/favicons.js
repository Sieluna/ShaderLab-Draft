...
class FaviconsPlugin {
    ...
    apply(compiler) {
        ...
        if (this.options.src && output.path) {
            ...
            compiler.hooks.thisCompilation.tap({ name: "FaviconsPlugin" }, compilation => {
                        ...
                        (error, response) => {
                                    ...
                                    (data, callback) => {
                                        ...
                                        Object.keys(response.html).map((i) => {
                                            ...
                                            Object.keys(attrs).map((j) => {
                                                ...
                                                if (key === "href" && compiler.options.output.publicPath !== "auto") {
                                                    attributes[key] = path.posix.join(compiler.options.output.publicPath, value);
                                                }
                                            });
                            ...
                            if (response.files) {
                                Object.keys(response.files).map((i) => {
                                    ...
                                    assets[path.normalize(`/${this.options.path}/${file.name}`).replace(/\\/g, "/")] = {
