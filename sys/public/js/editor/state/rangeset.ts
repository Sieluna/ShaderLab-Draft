...
export abstract class RangeValue {
     ...
     * are the same, to decide whether a change deletes the range. Defaults to `MapMode.TrackDel`.
     */
    mapMode!: MapMode
    /**
     * Determines whether this value marks a point range. Regular ranges affect the part of the
     * document they cover, and are meaningless when empty. Point ranges have a meaning on their
     * own. When non-empty, a point range is treated as atomic and shadows any ranges contained in it.
     */
    point!: boolean
...
/**
 * Methods used when iterating over the spans created by a set of ranges. The entire iterated
 * range will be covered with either `span` or `point` calls.
 */
export interface SpanIterator<T extends RangeValue> {
    /**
     * Called for any ranges not covered by point decorations. `active` holds the values that
     * the range is marked with (and may be empty). `openStart` indicates how many of those
     * ranges are open (continued) at the start of the span.
     */
...
export class RangeSet<T extends RangeValue> {
    ...
    // @internal
    static create<T extends RangeValue>(chunkPos: readonly number[], chunk: readonly Chunk<T>[], nextLayer: RangeSet<T>, maxPoint: number) {
