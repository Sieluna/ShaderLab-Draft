...
interface MarkDecorationSpec {
    // Whether the mark covers its start and end position or not. This influences whether
    // content inserted at those positions becomes part of the mark. Defaults to false.
    inclusive?: boolean
    // Specify whether the start position of the marked range should be inclusive.
    // Overrides `inclusive`, when both are present.
...
interface WidgetDecorationSpec {
    ...
    widget: WidgetType
    // Which side of the given position the widget is on. When this is positive, the widget
    // will be drawn after the cursor if the cursor is on the same position. Otherwise, it'll
    // be drawn before it. When multiple widgets sit at the same position, their `side` values
    // will determine their ordering—those with a lower value come first. Defaults to 0.
    side?: number
...
interface ReplaceDecorationSpec {
    ...
    widget?: WidgetType
    // Whether this range covers the positions on its sides. This influences whether new
    // content becomes part of the range and whether the cursor can be drawn on its sides.
    // Defaults to false for inline replacements, and true for block replacements.
    inclusive?: boolean
