import {EditorState, Transaction, ChangeSet, ChangeDesc, Facet, StateEffect, Extension,
        SelectionRange, RangeSet} from "../state/index"
import {StyleModule} from "../utils/style-mod"
...
/**
 * Command functions are used in key bindings and other types of user actions. Given an editor
 * view, they check whether their effect can apply to the editor, and if it can, perform it as
 * a side effect (which usually means [dispatching](#view.EditorView.dispatch) a transaction)
 * and return `true`.
 */
...
export interface PluginSpec<V extends PluginValue> {
    /**
     * Register the given [event handlers](#view.EditorView^domEventHandlers) for the plugin.
     * When called, these will have their `this` bound to the plugin value.
     */
